...
| ARCHITECTURE | ⚑ | MVP 高层流程 + 选型 + 禁改项已定，实现待 spike 通过后开发 |
| DEPLOYMENT | ⚑ | Tauri / Windows / 禁 360 已定，未部署（当前仅 spike 本地 Python）|
| PRD | ✅ | 产品意图 + 商业 / 合作 + 关键风险（蒸馏自需求沟通，敏感细节未入库）|
| ROADMAP | ✅ | Now = 第 0 步识别率 spike（gating）+ 学科 / 地域优先级 |
| CONVENTIONS | ✅ | 知识点 ID 规则 / 全角标点 / key 不入库 / 校准规则 |
| DECISIONS | ✅ | 7 条 ADR（2026-06-01 MVP 锁定决策）|
